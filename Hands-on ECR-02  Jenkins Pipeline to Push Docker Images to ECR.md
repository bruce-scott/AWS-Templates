...
Add "token" after "//" in the "url" part . And also paste "@" at the end of the token.
...
```bash
docker run --name todo -dp 80:3000 097771290285.dkr.ecr.us-east-1.amazonaws.com/clarusway-repo/todo-app:latest
```
...
```bash
aws ecr get-login-password --region us-east-1 | docker login --username AWS --password-stdin 097771290285.dkr.ecr.us-east-1.amazonaws.com
```
